...
tdcr = []                # results of ratio of calculated efficiencies
...
std_efficiency_T = np.std(efficiency_T)   # standard deviation
...
residual = (mean_TDCR-TDCR_measure)**2 # Calculation of the residual value to minimize to find the best free parameter L
u_residual = np.sqrt(4*(mean_TDCR+TDCR_measure)**2*(std_TDCR**2+u_TDCR_measure**2))  # uncertainty of the residual 
Z_test = abs(mean_TDCR-TDCR_measure)/np.sqrt(std_TDCR**2+u_TDCR_measure**2)          # statisitcal test to see if we can improve the estimation
...
# to discuss the meaning of this for the measurement 
