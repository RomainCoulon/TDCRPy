...
end_E = 2000                               # end for the deposited energy / keV
delta_E= 2                             # the step the distribution / keV
#npas = int((end_E - start_E)/delta_E)
energy_i = np.linspace(200, 2000, 901)        # emitted energy / keV
NPS = 1e6                                # number of source particlules for the MC calculation


for i in energy_i:
    with open("input2/input_"+str(int(i))+"keV.mcn", "w") as file:
        for j in range(1,44):
...
with open("script2.bat", "w") as file2:
    for i in energy_i:
        file2.write("mcnp6 i=input2/input_"+str(int(i))+"keV.mcn r=run/run_"+str(int(i))+"keV.r o=output2/output_"+str(int(i))+"keV.o\n")